...
import getCroppedImg, { isBase64Image } from "@/lib/utils";
...
const AddGalleryForm: React.FC = () => {
  ...
  const onSubmit = async (values: GalleryFormValues) => {
    try {
      ...
      reader.onloadend = async () => {
        ...
        const hasImage = isBase64Image(base64Image);
        if (hasImage) {
          const imageRes = await startUpload([blob]);
          if (imageRes && imageRes[0].url) {
            values.imageUrl = imageRes[0].url;
          }
          await addGalleryItem({imageUrl:values.imageUrl,title: values.title, path:path})
        }
      };
  ...
  const handleCropComplete = async (croppedAreaPixels: Area,
      fieldChange: (value: string) => void,imageUrl:string
    ) => {
      const croppedImg = await getCroppedImg(
        imageUrl,
        croppedAreaPixels!
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-6">
        <h2 className="text-center  " style={{fontWeight:"bold"}} > اضافة صوره</h2>
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="flex flex-col items-center gap-4">
              ...
              {field.value ? (
                 ...
                 value={field.value}
                 onCropComplete={
                 (e: Area)=> handleCropComplete(e,field.onChange,field.value)}
               />
              ) : (
                <></>
              )}
